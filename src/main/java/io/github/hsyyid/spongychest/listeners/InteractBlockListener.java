			...
			else if (player.hasPermission("spongychest.shop.create"))
			{
				...
				if (chestShopModifier.isPresent())
				{
					...
					if (itemFrame.isPresent())
					{
						...
						frameStack.offer(Keys.DISPLAY_NAME, Text.of(TextColors.GREEN, "Item: ", TextColors.WHITE, frameStack.getTranslation().get(), " ", 
							TextColors.GREEN, "Amount: ", TextColors.WHITE, frameStack.getQuantity(), " ", 
							TextColors.GREEN, "Price: ", TextColors.WHITE, SpongyChest.economyService.getDefaultCurrency().getSymbol().toPlain(), chestShopModifier.get().getPrice()));
